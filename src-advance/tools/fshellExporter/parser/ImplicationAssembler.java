...
//			target.push( Boolean.FALSE );
		...
		Object rs = target.pop();
		Object ls = target.pop();
		
		if ( rs instanceof NonExistentVariable  ){
			System.err.println("Unexpected unexistet variable in implication : "+ ((NonExistentVariable)rs).getVariableName() );
			target.push(Boolean.FALSE);
			ImplicationStateRegistry.getInstance().reset();
			return;
		}
		
		if ( ls instanceof NonExistentVariable  ){
			System.err.println("Unexpected unexistet variable in implication : "+ ((NonExistentVariable)ls).getVariableName() );
			target.push(Boolean.FALSE);
			ImplicationStateRegistry.getInstance().reset();
			return;
		}
		...
		String rightSide = (String) rs.toString(); 
		String leftSide = (String)  ls.toString();
		
		String implication = "! ("+leftSide+") || ("+leftSide+" && "+rightSide+")";
//		System.out.println("IMPLICATION: "+implication);
		target.push( implication );
