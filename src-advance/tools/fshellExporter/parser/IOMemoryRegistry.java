...
public class IOMemoryRegistry {
	...
	private static HashMap<Long,IOMemoryRegistry> threads = null;
	//Stack<HashMap>
	private Stack<HashMap<String,Object>>  stack = null;
	/**
	...
	public static synchronized IOMemoryRegistry getInstance(){
		...
		if ( threads == null )
			threads = new HashMap<Long,IOMemoryRegistry>();
		IOMemoryRegistry instance = (IOMemoryRegistry)threads.get( Long.valueOf(tid));
		if ( instance == null ){
			instance = new IOMemoryRegistry();
			threads.put( Long.valueOf(tid),instance);
		}
		return instance;
	}
