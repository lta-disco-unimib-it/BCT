...
public class BctLA28 {
  ...
  public static class Exec {
	  ...
	  public void logEntry(final JoinPoint joinPoint) {
		  //System.out.println("ENTER "+BctLA28.Exec.class.getName()+" "+getSignature(joinPoint));
		  try {	
			  ComponentCallStack s = ComponentCallStack.getInstance();
			  if ( BctLA28.Exec.class == s.lastElement() ){
				  s.push( BctLA28.Exec.class );
				  //System.out.println("ENTER#"+BctLA28.Exec.class.getName()+" "+getSignature(joinPoint));
				  return;
			  }
			  s.push( BctLA28.Exec.class );
	  ...
	  public void logExit(final JoinPoint joinPoint) {
		  //System.out.println("EXIT "+BctLA28.Exec.class.getName()+" "+getSignature(joinPoint));
		  try {
			  ComponentCallStack s = ComponentCallStack.getInstance();
			  if ( s.lastElement() != BctLA28.Exec.class )
				  System.out.println("POPERROR "+BctLA28.Exec.class.getName()+" "+getSignature(joinPoint));
  ...
  public static class Call {
	  ...
	  public void logEntry(final JoinPoint joinPoint) {
		  //System.out.println("ENTER "+BctLA28.Call.class.getName()+" "+getSignature(joinPoint));
		  try {	
			  ComponentCallStack s = ComponentCallStack.getInstance();
			  s.push( BctLA28.Call.class );
			  logIO(joinPoint, ENTER);
			  LoggingActionRecorder.logInteractionEnter(getSignature(joinPoint), Thread.currentThread().getId() );
		  } catch ( Exception e ){
			  e.printStackTrace();
	  ...
	  public void logExit(final JoinPoint joinPoint) {
		  //System.out.println("EXIT "+BctLA28.Call.class.getName()+" "+getSignature(joinPoint));
		  try {
