...
public class BctLA45 {
  ...
  public static class Exec {
	  ...
	  public void logEntry(final JoinPoint joinPoint) {
		  //System.out.println("ENTER "+BctLA45.Exec.class.getName()+" "+getSignature(joinPoint));
		  try {	
			  ComponentCallStack s = ComponentCallStack.getInstance();
			  if ( BctLA45.Exec.class == s.lastElement() ){
				  s.push( BctLA45.Exec.class );
				  //System.out.println("ENTER#"+BctLA45.Exec.class.getName()+" "+getSignature(joinPoint));
				  return;
			  }
			  s.push( BctLA45.Exec.class );
	  ...
	  public void logExit(final JoinPoint joinPoint) {
		  //System.out.println("EXIT "+BctLA45.Exec.class.getName()+" "+getSignature(joinPoint));
		  try {
			  ComponentCallStack s = ComponentCallStack.getInstance();
			  if ( s.lastElement() != BctLA45.Exec.class )
				  System.out.println("POPERROR "+BctLA45.Exec.class.getName()+" "+getSignature(joinPoint));
  ...
  public static class Call {
	  ...
	  public void logEntry(final JoinPoint joinPoint) {
		  //System.out.println("ENTER "+BctLA45.Call.class.getName()+" "+getSignature(joinPoint));
		  try {	
			  ComponentCallStack s = ComponentCallStack.getInstance();
			  s.push( BctLA45.Call.class );
			  logIO(joinPoint, ENTER);
			  LoggingActionRecorder.logInteractionEnter(getSignature(joinPoint), Thread.currentThread().getId() );
		  } catch ( Exception e ){
			  e.printStackTrace();
	  ...
	  public void logExit(final JoinPoint joinPoint) {
		  //System.out.println("EXIT "+BctLA45.Call.class.getName()+" "+getSignature(joinPoint));
		  try {
