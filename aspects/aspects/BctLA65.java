...
public class BctLA65 {
  ...
  public static class Exec {
	  ...
	  public void logEntry(final JoinPoint joinPoint) {
		  //System.out.println("ENTER "+BctLA65.Exec.class.getName()+" "+getSignature(joinPoint));
		  try {	
			  ComponentCallStack s = ComponentCallStack.getInstance();
			  if ( BctLA65.Exec.class == s.lastElement() ){
				  s.push( BctLA65.Exec.class );
				  //System.out.println("ENTER#"+BctLA65.Exec.class.getName()+" "+getSignature(joinPoint));
				  return;
			  }
			  s.push( BctLA65.Exec.class );
	  ...
	  public void logExit(final JoinPoint joinPoint) {
		  //System.out.println("EXIT "+BctLA65.Exec.class.getName()+" "+getSignature(joinPoint));
		  try {
			  ComponentCallStack s = ComponentCallStack.getInstance();
			  if ( s.lastElement() != BctLA65.Exec.class )
				  System.out.println("POPERROR "+BctLA65.Exec.class.getName()+" "+getSignature(joinPoint));
  ...
  public static class Call {
	  ...
	  public void logEntry(final JoinPoint joinPoint) {
		  //System.out.println("ENTER "+BctLA65.Call.class.getName()+" "+getSignature(joinPoint));
		  try {	
			  ComponentCallStack s = ComponentCallStack.getInstance();
			  s.push( BctLA65.Call.class );
			  logIO(joinPoint, ENTER);
			  LoggingActionRecorder.logInteractionEnter(getSignature(joinPoint), Thread.currentThread().getId() );
		  } catch ( Exception e ){
			  e.printStackTrace();
	  ...
	  public void logExit(final JoinPoint joinPoint) {
		  //System.out.println("EXIT "+BctLA65.Call.class.getName()+" "+getSignature(joinPoint));
		  try {
