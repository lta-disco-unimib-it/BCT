...
//			------------------ begin user-written fragment code ----------------
			try{
				//System.out.println("#"+Thread.currentThread().getId()+"#"+"CALLEXIT"+bctLP1.class);
				ComponentCallStack.getInstance().pop();
				LoggerProbe.exit(className,methodName,methodSig,args,returnedObject,calledObject);
...
//			------------------- end user-written fragment code -----------------
		...
		public static void _beforeCall (
			...
			try{
				//System.out.println("#"+Thread.currentThread().getId()+"#"+"CALLENTER"+bctLP1.class);
				ComponentCallStack s = ComponentCallStack.getInstance();
				s.push( bctLP1.Probe_0.class );

				LoggerProbe.enter(className,methodName,methodSig,args,calledObject);
			} catch ( Throwable e ){
				System.err.println("BCT COMPONENT LOGGER ERROR/EXCEPTION: "+e.getMessage());
				e.printStackTrace();
			}
	...
	public static class Probe_1 {
		...
		public static void _exit (
			...
			try{
				...
				if ( bctLP1.Probe_1.class == s.lastElement() ){
