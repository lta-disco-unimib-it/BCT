...
private static Parser basicExpression(){
  ...
  private static Parser invariantParser() {
   ...
   private static Assembly createTokenAssembly( String expression, String programPoint) {
     ...
     */
     ...
     a.setTarget(new Target(programPoint));
     
     return a;
   }

   public static ArrayList<TcSpecification> getTestCaseSpecifications(String expression,String programPoint) throws InvariantParseException {
	   //System.out.println("evaluateExpression: "+expression);  
	 currentExpression = expression; 
	 //System.out.println(expression);
	 
    Parser p = invariantParser();
    Assembly a = createTokenAssembly(expression,programPoint);      

    
    //Assembly result = p.completeMatch(a);
    
    Assembly result = p.bestMatch(a);
    
    if(result==null) {
      throw new InvariantParseException("Parsing error on expression "+expression);
    } else {
      Target resultingTarget = (Target)result.getTarget();
      ArrayList<TcSpecification> tcSpecs = new ArrayList<TcSpecification>(resultingTarget.queueSize());
      
      while ( resultingTarget.queueSize() > 0 ){
    	  
    	  TcSpecification element = (TcSpecification) resultingTarget.pop();
    	  
    	  //if it is a block extract its elements
    	  if ( element instanceof TcSpecificationBlock ){
    		  Iterator<TcSpecification> blockElements =  ((TcSpecificationBlock)element).elementsIterator();
    		  while ( blockElements.hasNext() ){
    			  tcSpecs.add( blockElements.next() );
    		  }
    	  } else {
    		  tcSpecs.add( element );
    	  }
