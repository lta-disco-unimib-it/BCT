...
public class InteractionTrace {
	...
	public static void insert(String methodName, List<Token> trace, String threadId) throws DataLayerException {
		...
		if(trace != null) {
			...
			for (int currentPosition = 0; currentPosition < trace.size(); currentPosition++) {	
				occurrence++;
				try {
					MethodCall.insert(idInteractionTrace, ((Token)trace.get(currentPosition)).getTokenValue().substring(0, ((Token)trace.get(currentPosition)).getTokenValue().length()-1), occurrence, ((Token)trace.get(currentPosition)).getId());
