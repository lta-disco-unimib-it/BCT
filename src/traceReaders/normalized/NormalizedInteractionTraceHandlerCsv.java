...
public class NormalizedInteractionTraceHandlerCsv implements
...
//	private TraceRepository usageInteractionRepository;
	 ...
	 */
	public NormalizedInteractionTraceHandlerCsv( File interactionOutputFolder ){
		this(interactionOutputFolder, ".txt");
	}
	
	public NormalizedInteractionTraceHandlerCsv( File interactionOutputFolder, String traceSuffix ){
		this.interactionOutputFolder = interactionOutputFolder;
		interactionRepository = new TraceRepository( this.interactionOutputFolder, true, traceSuffix );
	...
	protected void addTrace(String methodName, List<Token> trace, TraceRepository traceRepository ) throws NormalizedTraceHandlerException{
		try {
			if(trace == null) {
				traceRepository.addTrace(methodName, lineSeparator+"|"+lineSeparator);
			} else {
				...
				while ( it.hasNext() ){
					...
					if ( currentPosition > 0 ){
						methodCallsBuf.append(lineSeparator);
					}
				...
				methodCallsBuf.append(lineSeparator+"|"+lineSeparator);
	...
	public void cleanup() {
		...
		interactionRepository = new TraceRepository( this.interactionOutputFolder, true );
	}
