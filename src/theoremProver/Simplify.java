...
public class Simplify {
	...
	public ArrayList<SimplifyResult> isValid(List<String> assumptions, String predicate) throws IOException {
		StringBuffer lb = new StringBuffer();
	
		lb.append("(AND ");
		for( String assumption : assumptions ){
			if ( ! assumption.trim().startsWith("(") ){
				assumption = "("+assumption+")";
			}
			lb.append(assumption);
		}
		lb.append(")");
		
		return doIMPLIES(lb.toString(), predicate);
	...
	public static void main(String[] args) {		
		...
		try {						
			...
			//= "(NEQ (select |intValue()| (select (select elems |parameter|) 0)) (select |intValue()| (select (select elems |__orig__parameter|) 0))) (EQ (select |intValue()| (select (select elems |parameter|) 0)) 0)";
			//String formula = "";
