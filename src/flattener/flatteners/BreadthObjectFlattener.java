...
public class BreadthObjectFlattener implements Flattener {
	...
	private static class NodeRecorderRaw implements NodeRecorder{
		...
		public void recordArray(Object array, ObjectInfo objectInfo) {
			dataHandler.addArrayNode( objectInfo.getInspectorFullName(), array );
		}
	...
	public static boolean skipFakeJavaInspectors = true;	       
	...
	private Hashtable classesToIgnore = new Hashtable();
	...
	private boolean smashWholeArray = false;

	private boolean skipAlreadyVisited;
	...
	private void smashPrimitiveWrapper( Object object, ObjectInfo objInfo ){
		...
		if ( skipFakeJavaInspectors ){
			nodeRecorder.recordField(objInfo.getInspectorFullName(), object);
			return;
		}
	...
	private void smashFirst(Object object) {
		...
		do{
			while ( ! nodeHandler.curLevelEmpty() ){
				...
				if ( skipAlreadyVisited && currentObj != null && elaboratedObjects.containsKey(currentObj)) {
					addRef(element);
				} else {
	...
	private void smash(Object object, ObjectInfo objInfo) {
		...
		if (isPrimitiveArray(object)) { 
			if (smashAggregations){
				if ( smashWholeArray ){
					nodeRecorder.recordArray(object, objInfo );
					return;
				}
	...
	private void flattenFields(Object object,ObjectInfo objectInfo) {
		...
		//List<Field> fields = IdManager.getFieldsMeth(object,fieldsFilter);
		//List<Field> fields = IdManager.getFields(object,fieldsFilter);
		...
		if ( object instanceof Throwable ){
			nodeHandler.addFieldChildren(object.getClass().getCanonicalName(), objectInfo, "getClass().getCanonicalName()" );
	...
	private boolean isIgnoredClassType(Class objectClass){
		...
		String value = (String)classesToIgnore.get( objectClass.getName() );
	...
	private void loadProperties() {
		...
		this.classesToIgnore = pl.getClassesToIgnoreList();
		this.fieldsRetriever = pl.getFieldsRetriever();
		this.smashWholeArray = pl.isSmashArraysAsAWhole();
		this.skipAlreadyVisited = pl.isSkipObjectsAlreadyVisited();
	}
	...
	public void setSkipAlreadyVisited(boolean skipAlreadyVisited) {
		this.skipAlreadyVisited = skipAlreadyVisited;
	}
