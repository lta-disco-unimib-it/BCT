...
public class DaikonStimuliRecorder implements StimuliRecorder {	
	 ...
	 */
	public long record(Object object) throws ClassCastException, IOException {
		currentPath = new String();		

		Document root = (Document) object;	
					
		return explore(root.getDocumentElement());
	}
	 ...
	 * @throws IOException
	 */
	private long explore(Element element) throws IOException {
		...
		if (element.getAttributeNode("value") != null && element.getAttributeNode("type") != null) {
			...
			writtenDataSize+=message.length();
			this.writer.flush();			
