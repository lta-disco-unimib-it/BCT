...
public class Superstructure implements Serializable  {
	...
	private ArrayList<String> alwaysNotNull = null;//this must be null at the beginning 
	...
	private void add ( String varName, VarTypeResolver.Types type, boolean fakeHash, boolean isArray ){
		...
		if ( var == null ){
			...
			if ( var.getVarType() != type ){
				...
				if ( var.getVarType() == VarTypeResolver.Types.integerType && type == VarTypeResolver.Types.doubleType  ){
					var.setVarType(type);
					var.setArray(isArray);
				}

				//all other types have the same importance
				//TODO: show WARNING when different types for the same variable

			}
		}
		structure.put( varName, var );
	}
	...
	public void expandReferences(){
		...
		while ( ! references.isEmpty() ){
			...
			if ( referencer.equals(referenced) ){
				//fixes bug#153
				//in case of inner objects the flattener put references to the object itself
				//eg. returnValue.this$0
				//    @returnValue.this$0
				continue; 
	...
	/**
	 * Remove the field from the superstructure
	 * 
	 * @param field
	 */
	public void remove(SuperstructureField field) {
		structure.remove(field.getVarName());
	}

	public SuperstructureField getField( String fieldName ){
