...
public class IoInvariantParser  {
	...
	private static boolean offlineAnalysis;
	...
	public static boolean isOfflineAnalysis() {
		return offlineAnalysis;
	}
	public static void setOfflineAnalysis(boolean offlineAnalysis) {
		IoInvariantParser.offlineAnalysis = offlineAnalysis;
	}
...
//	private static Parser localVariable(){
//		Repetition stars = new Repetition(new Symbol('*').setAssembler(new StarAssembler()));
//		
//		Parser p = new Sequence()
//		.add(stars)
//		.add(new Word().setAssembler(new WordAssembler()));
//		return p;
//	}
...
//		.add(localVariable())
	...
	private static Assembly createTokenAssembly(String expression,Object[] parameters,Object returnValue, Map<String, Object> localVariables) {
     ...
     t.setCharacterState(0xc0,0xff,wfis);
		...
		a.setTarget(new Target(parameters,returnValue, localVariables, expression));
		
		return a;
	}

	public static boolean evaluateExpression(String expression,Object[] parameters,Object returnValue, Map<String, Object> localVariables) throws InvariantParseException {
//		System.out.println("#evaluateExpression: "+expression);  

		//System.out.println(expression);
		if ( offlineAnalysis ){
			return IoInvariantParserOffline.evaluateExpression(expression, parameters, returnValue, localVariables);
		}
		
		Parser p = invariantParser();
		Assembly a = createTokenAssembly(expression,parameters,returnValue, localVariables );      

		if ( offlineAnalysis &&  ( localVariables != null ) ){
			...
			
			{
				...
				for ( String key : toReplace ){
					localVariables.put(key, new LocalVariablesWrapper(key, null, target) );
				}
			}
			
			{
				ArrayList<String> toReplace = new ArrayList<String>();
				for ( Entry<String, Object> e : localVariables.entrySet() ){
					if( "NULL".equals( e.getValue()) || "null".equals( e.getValue()) ){
						toReplace.add(e.getKey());
					}
				}

				for ( String key : toReplace ){
					localVariables.put(key, null );
				}
			}
		}
	...
	public static void main(String argv[]) throws Exception {
		...
		System.out.println(exp + " = " + evaluateExpression(exp,parameters,"",null));
	}  
}