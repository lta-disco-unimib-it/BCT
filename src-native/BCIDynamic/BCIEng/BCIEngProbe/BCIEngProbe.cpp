 ...
 * $Id: BCIEngProbe.cpp,v 1.1.2.4 2007-10-25 15:39:44 pastore Exp $ 
 * 
 * Contributors: 
...
//==============================================================================
//------------------------------------------------------------------------------
void	CProbe::AddFilterRule( CFilterRule& rule ){
	CFilterRule fr(rule);
	m_ruleList.push_back(fr);
};

void	CProbe::AddFilterRuleWithin( CFilterRule& rule ){
	CFilterRule fr(rule);
...
void
CProbe::AddPrefilterRule(CSTR i_szPackageName, 
					  CSTR i_szClassName, 
					  CSTR i_szMethodName, 
					  CSTR i_szMethodSig, 
					  CFilterRule::action_t i_action)
...
void
CProbe::AddPrefilterRuleWithin(CSTR i_szPackageName, 
					  CSTR i_szClassName, 
					  CSTR i_szMethodName, 
					  CSTR i_szMethodSig, 
					  CFilterRule::action_t i_action)
{
	CFilterRule fr(i_szPackageName, i_szClassName, i_szMethodName, i_szMethodSig, i_action);
	m_plstPrefilterWithin->push_back(fr);
}
...
{
	return MatchCommon( m_ruleList, m_plstPrefilter, i_szPkgAndClass, i_szMethod, i_szSig, pIsExplicit );
}

bool
CProbe::MatchWithin(CSTR i_szPkgAndClass, CSTR i_szMethod, CSTR i_szSig, bool* pIsExplicit /* = NULL */)
{
	return MatchCommon( m_ruleListWithin, m_plstPrefilterWithin, i_szPkgAndClass, i_szMethod, i_szSig, pIsExplicit );
}
								 ...
								 CJClassBuilder* pClass)
{
	...
	//for BCT instrumentation we consider only explicit matches 
	bool isExplicit;
	bool matchSucceeded = (pProbe->*matchFunction)(className, methodName, methodSig, &isExplicit );
	
	if ( matchSucceeded && isExplicit ) {
		return true;
	}
	return false;
		/* for BCT we do not want to consider the implemented interfaces
		else {
				 ...
				 interface_iter++)
			{
				...
				//bool isExplicit;
				matchSucceeded = (pProbe->*matchFunction)(interface_name.c_str(), methodName, methodSig, &isExplicit);
				if (matchSucceeded && isExplicit) {
					return true;
				}
			}
						...
						BlockInsertionStash& block_insertion_stash)
{
	...
	cout << "CallSite insertion : " << ctxt.methodName << endl;
...
{
	cout << "Instrumenting Method: " << i_pmeth->GetName() << endl;
