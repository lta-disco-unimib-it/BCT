...
public class RegressionInvariantGeneratorTestT1 extends TestCase {

	public void testT1() throws IOException{
		
		RegressionInvariantGeneratorTest.setTestHome("T1");
		...
		vector = it.next();
		assertTrue(RegressionInvariantGeneratorTest.vectorsEqual(new boolean[]{true,false,false},vector));
		...
		vector = it.next();
		assertTrue(RegressionInvariantGeneratorTest.vectorsEqual(new boolean[]{false,false,true},vector));
		
		
		
		is.close();
		
		RegressionInvariantGeneratorUtil.clean("T1");
	}
}
